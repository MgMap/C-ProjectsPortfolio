...
void MainMenuState::init()
{
    ...
    _title.setPosition((PORTFOLIO_SCREEN_WIDTH / 2) - (_title.getGlobalBounds().width / 2),
                       _title.getGlobalBounds().height / 2);
    _playButton.setPosition((PORTFOLIO_SCREEN_WIDTH / 2) -
                            (_playButton.getGlobalBounds().width / 2),
                            (PORTFOLIO_SCREEN_HEIGHT/2)-_playButton.getGlobalBounds().height / 2);
